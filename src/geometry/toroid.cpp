...
Toroid::Toroid(Point3 cen, Vec3 ax, double R_in, double r_in, std::shared_ptr<Material> m)
...
bool Toroid::hit(const Ray& ray, double t_min, double t_max, HitRecord& rec) const {
...
bool Toroid::bounding_box(AABB& output_box) const {
    double rad = R + r;
